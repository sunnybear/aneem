...
# приведение дат
            ...
            data[col] = pd.to_datetime(data[col].fillna('').replace('202-','2024-').replace('', '2000-01-01T00:00:00+03:00').apply(lambda x: dt.strptime(x, '%Y-%m-%dT%H:%M:%S%z').strftime("%Y-%m-%d %H:%M:%S")))
# приведение строк
        else:
