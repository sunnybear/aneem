# Скрипт для первоначального получения таблицы воронок из AmoCRM
...
pipelines_exists = True
page = 1
statuses = {}

while pipelines_exists:
# отправка запроса
...
# разбор ответа в статусы воронки со всеми полями
        ...
        if table_not_created:
            if config["DB"]["TYPE"] == "CLICKHOUSE":
                requests.post('https://' + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':8443/', verify=False,
                    params={"database": config["DB"]["DB"], "query": (pd.io.sql.get_schema(data, config["AMOCRM"]["TABLE_PIPELINES"]) + "  ENGINE=ReplacingMergeTree ORDER BY (`id`)").replace("CREATE TABLE ", "CREATE OR REPLACE TABLE " + config["DB"]["DB"] + ".").replace("INTEGER", "Int64")})
            else:
                connection.execute(text("DROP TABLE IF EXISTS " + config["AMOCRM"]["TABLE_PIPELINES"]))
                connection.commit()
...
if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
    connection.execute(text("ALTER TABLE " + config["AMOCRM"]["TABLE_PIPELINES"] + " ADD INDEX pipeline_id (`pipeline_id`)"))
    connection.execute(text("ALTER TABLE " + config["AMOCRM"]["TABLE_PIPELINES"] + " ADD INDEX id (`id`)"))
    connection.commit()
    connection.close()