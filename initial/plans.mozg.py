# Скрипт для первоначального получения всех данных по планам продаж из Mozg.rest
...
# * MOZG.TABLE_PLANS - имя результирующей таблицы для планов продаж
...
result = requests.post(config['MOZG']['API_ENDPOINT'],
    files = {'orgid': (None, config['MOZG']['ORGID']), 'dbid': (None, config['MOZG']['DBID']), 'impkey': (None, config['MOZG']['IMPKEY']),  'type': (None, 'Plans'), 'fields': (None, 'SourceCode,RestCode,Date,ValueType,Value,Correct')})

# формируем датафрейм из ответа API
data = pd.read_csv(StringIO(result.text), delimiter=";", names = ['SourceCode', 'RestCode', 'Date', 'ValueType', 'Value', 'Correct'])
# базовый процесс очистки: приведение к нужным типам
for col in data.columns:
# приведение целых чисел
    if col in ["SourceCode"]:
        data[col] = data[col].fillna(0).replace('', 0).astype(np.int64)
# приведение вещественных чисел
    elif col in ["Value", "Correct"]:
...
# создаем таблицy
    if config["DB"]["TYPE"] == "CLICKHOUSE":
        requests.post(CLICKHOUSE_PROTO + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':' + CLICKHOUSE_PORT + '/', verify=False,
            params={"database": config["DB"]["DB"], "query": (pd.io.sql.get_schema(data, config["MOZG"]["TABLE_PLANS"]) + "  ENGINE=MergeTree ORDER BY (`SourceCode`)").replace("CREATE TABLE ", "CREATE OR REPLACE TABLE " + config["DB"]["DB"] + ".").replace("INTEGER", "Int64")})
    if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
# обработка ошибок при добавлении данных
        try:
            data.to_sql(name=config["MOZG"]["TABLE_PLANS"], con=engine, if_exists='append', chunksize=100)
        except Exception as E:
            print (E)
            connection.rollback()
        ...
        requests.post(CLICKHOUSE_PROTO + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':' + CLICKHOUSE_PORT + '/',
            params={"database": config["DB"]["DB"], "query": 'INSERT INTO ' + config["DB"]["DB"] + '.' + config["MOZG"]["TABLE_PLANS"] + ' FORMAT CSV'},
...
if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
# добавление индексов
    connection.execute(text("ALTER TABLE " + config["MOZG"]["TABLE_PLANS"] + " ADD INDEX dateidx (`Date`)"))
    connection.commit()
    connection.close()