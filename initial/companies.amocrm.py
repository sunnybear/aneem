# Скрипт для первоначального получения таблицы компаний из AmoCRM
...
# отправка запроса
    ...
    if '_links' not in result or 'next' not in result['_links']:
        companies_exists = False
# разбор ответа в компанию со всеми полями
    if len(result['_embedded']['companies']):
        company_new = result['_embedded']['companies']
        for l in company_new:
            company = {}
            for k in l.keys():
        ...
        if table_not_created:
            if config["DB"]["TYPE"] == "CLICKHOUSE":
                requests.post('https://' + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':8443/', verify=False,
                    params={"database": config["DB"]["DB"], "query": (pd.io.sql.get_schema(data, config["AMOCRM"]["TABLE_COMPANIES"]) + "  ENGINE=ReplacingMergeTree ORDER BY (`id`)").replace("CREATE TABLE ", "CREATE OR REPLACE TABLE " + config["DB"]["DB"] + ".").replace("INTEGER", "Int64")})
            else:
                connection.execute(text("DROP TABLE IF EXISTS " + config["AMOCRM"]["TABLE_COMPANIES"]))
                connection.commit()
...
# обработка ошибок при добавлении данных
            try:
                data.to_sql(name=config["AMOCRM"]["TABLE_COMPANIES"], con=engine, if_exists='replace', chunksize=100)
            except Exception as E:
                print (E)
                connection.rollback()
...
if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
    connection.execute(text("ALTER TABLE " + config["AMOCRM"]["TABLE_COMPANIES"] + " ADD INDEX updated_at (`updated_at`)"))
    connection.execute(text("ALTER TABLE " + config["AMOCRM"]["TABLE_COMPANIES"] + " ADD INDEX created_at (`created_at`)"))
    connection.execute(text("ALTER TABLE " + config["AMOCRM"]["TABLE_COMPANIES"] + " ADD INDEX id (`id`)"))
    connection.commit()
    connection.close()