# Скрипт для ежедневного обновления данных по компаниям Битрикс24 (crm.company) для облачных функций Яндекс.Облака
...
# * BITRIX24_TABLE_COMPANIES - имя таблицы с контактами
...
# формируем запрос для получения общего количества измененных компаний
    companies_req = requests.get(os.getenv('BITRIX24_WEBHOOK') + 'crm.company.list.json?ORDER[ID]=ASC&FILTER[>DATE_MODIFY]=' + yesterday).json()
    companies = {}
    last_company_id = '0'
...
# удаление старых данных
            if os.getenv('DB_TYPE') in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
                connection.execute(text("DELETE FROM " + os.getenv('BITRIX24_TABLE_COMPANIES') + " WHERE ID IN (" + ",".join(ids) + ")"))
                connection.commit()
            elif os.getenv('DB_TYPE') == "CLICKHOUSE":
                requests.post("https://" + os.getenv('DB_HOST') + ":8443/?database=" + os.getenv('DB_DB') + "&query=DELETE FROM " + os.getenv('DB_PREFIX') + "." + os.getenv('BITRIX24_TABLE_COMPANIES') + " WHERE ID IN (" + ",".join(ids) + ")",
                    headers=auth_post, verify=cacert)
# добавление новых данных
            if os.getenv('DB_TYPE') in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
# обработка ошибок при добавлении данных
                try:
                    data.to_sql(name=os.getenv('BITRIX24_TABLE_COMPANIES'), con=engine, if_exists='append', chunksize=100)
                    connection.commit()
                ...
                requests.post('https://' + os.getenv('DB_HOST') + ':8443', headers=auth_post, verify=cacert,
                    params={"database": os.getenv('DB_DB'), "query": 'INSERT INTO ' + os.getenv('DB_PREFIX') + '.' + os.getenv('BITRIX24_TABLE_COMPANIES') + ' FORMAT CSV'},
