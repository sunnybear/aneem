...
# формируем запрос для получения метрик яз Яндекс.Директа за последние 150 дней
    api = YandexDirect(
        access_token=os.getenv('YANDEX_DIRECT_ACCESS_TOKEN'),
        login=os.getenv('YANDEX_DIRECT_ACCESS_EMAIL'),
        is_sandbox=False,
...
# удаление старых данных
        if os.getenv('DB_TYPE') in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
            try:
                connection.execute(text("DELETE FROM " + os.getenv('YANDEX_DIRECT_TABLE') + " WHERE `Date`>='" + date_since + "'"))
                connection.commit()
...
# удаление старых данных
            requests.post('https://' + os.getenv('DB_HOST') + ':8443', headers=auth, verify=cacert,
                params={"database": os.getenv('DB_DB'), "query": "DELETE FROM " + os.getenv('DB_PREFIX') + "." + os.getenv('YANDEX_DIRECT_TABLE') + " WHERE `Date`>='" + date_since + "'"})
