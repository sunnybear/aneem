# Скрипт для ежедневного обновления контактов из AmoCRM
...
# разбор ответа в контакт со всеми полями
    if result and len(result['_embedded']['contacts']):
        for l in result['_embedded']['contacts']:
            contact = {}
            for k in l.keys():
...
# формируем датафрейм
    ...
    ids = ','.join(map(str, list(contacts.keys())))
...
# сверка списка полей
        if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
            current_columns = pd.read_sql("SELECT * FROM " + config["AMOCRM"]["TABLE_CONTACTS"] + " WHERE ID=" + ids[0], connection).columns
        elif config["DB"]["TYPE"] == "CLICKHOUSE":
            current_columns = pd.read_csv(StringIO(requests.get('https://' + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':8443/', params={"database": config["DB"]["DB"], "query": 'SHOW COLUMNS FROM ' + config["DB"]["DB"] + '.' + config["AMOCRM"]["TABLE_CONTACTS"]}, verify=False).text), delimiter="\t", header=None)[0]
# добавление новых столбцов
        for column_new in set(data.columns)-set(current_columns):
            if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
                connection.execute(text("ALTER TABLE " + config["AMOCRM"]["TABLE_CONTACTS"] + " ADD COLUMN `" + column_new + "` (TEXT)"))
                connection.commit()
            elif config["DB"]["TYPE"] == "CLICKHOUSE":
                requests.post('https://' + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':8443/',
                params={"database": config["DB"]["DB"], "query": 'ALTER TABLE ' + config["DB"]["DB"] + '.' + config["AMOCRM"]["TABLE_CONTACTS"] + ' ADD COLUMN `' + column_new + '` String'}, verify=False)
        if config["DB"]["TYPE"] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
# удаление старых данных
            try:
                connection.execute(text("DELETE FROM " + config["AMOCRM"]["TABLE_CONTACTS"] + " WHERE id IN (" + ids  +")"))
                connection.commit()
# добавление новых данных
                try:
                    data.to_sql(name=config["AMOCRM"]["TABLE_CONTACTS"], con=engine, if_exists='append', chunksize=100)
                except Exception as E:
                    print (E)
                    connection.rollback()
            except Exception as E:
...
# добавление/замена (ReplacingMergeTree) новых данных
            ...
            requests.post('https://' + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':8443/', verify=False,
                params={"database": config["DB"]["DB"], "query": "OPTIMIZE TABLE " + config["DB"]["DB"] + "." + config["AMOCRM"]["TABLE_CONTACTS"])
