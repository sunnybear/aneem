# Скрипт для ежедневного удаление контантов, удаленных из AmoCRM
...
chunk = 0
if config['DB']['TYPE'] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
    ids = list(pd.read_sql('SELECT id FROM ' + config['AMOCRM']['TABLE_CONTACTS'] + ' WHERE is_deleted=0 ORDER BY id DESC LIMIT ' + str(chunk) + ', 1000', connection)["id"].values)
elif config['DB']['TYPE'] == "CLICKHOUSE":
    ids = requests.get('https://' + config["DB"]["USER"] + ':' + config["DB"]["PASSWORD"] + '@' + config["DB"]["HOST"] + ':8443/?query=SELECT id FROM ' + config['DB']['DB'] + '.' + config['AMOCRM']['TABLE_CONTACTS'] + ' FINAL WHERE is_deleted=0 ORDER BY id DESC LIMIT ' + str(chunk) + ', 1000', verify=False).text.strip().split("\n")

# проверяем каждую сделку по id на наличие в Amo.CRM
for id in ids:
    lead = requests.get('https://' + config['AMOCRM']['INSTANCE'] + '/api/v4/contacts/' + str(id),
...
# 2 повторные проверки, чтобы исключить случайные/сетевые сбои
        ...
        if len(lead.text) == 0:
            ...
            if len(lead.text) == 0:
                if config['DB']['TYPE'] in ["MYSQL", "POSTGRESQL", "MARIADB", "ORACLE", "SQLITE"]:
                    connection.execute(text('UPDATE ' + config["DB"]["DB"] + '.' + config["AMOCRM"]["TABLE_CONTACTS"] + ' SET is_deleted=1 WHERE id=' + id))
                    connection.commit()
                elif config['DB']['TYPE'] == "CLICKHOUSE":
